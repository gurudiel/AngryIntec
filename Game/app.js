...
var AngryMenu = cc.Sprite.extend({
    escalaPorDefecto: 0.7,
    flote: false,
    bBox: null,
    alRetornarClick: null,
    ctor: function (tex) {
        this._super();
        this.initWithTexture(tex);
        this.setScale(this.escalaPorDefecto);
    },
    alClick: function (callback) {
        this.alRetornarClick = callback;
    },
    alToque: function (touch, evt) {
        (this.flote && this.alRetornarClick) && this.alRetornarClick();
    },
    movidoAlToque: function (touch, evt) {
        var point = touch[0].getLocation();

        this.bBox || (this.bBox = this.getBoundingBox());

        if (cc.Rect.CCRectContainsPoint(this.bBox, point)) {
            if (!this.flote) {
                this.flote = true;
            ...
            this.flote = false;
            this.runAction(cc.ScaleTo.create(0.01, this.escalaPorDefecto));
        }
    },
    terminadoAlToque: function (touch, evt) {}
});
...
var HelloWorldLayer = cc.Layer.extend({
    ...
    init: function () {
        ...
        var margin = 25,
            backMenu = new AngryMenu(this.getTexture("menu_back"));
        backMenu.setPosition(cc.p(margin, winSize.height - margin));
        backMenu.alClick(function () {
            window.location.href = "http://www.intec.edu.do";
        });
        this.addChild(backMenu);
        this.menus.push(backMenu);

        var refreshMenu = new AngryMenu(this.getTexture("menu_refresh"));
        refreshMenu.setPosition(cc.p(70, winSize.height - margin));
        refreshMenu.alClick(function () {
            self.init();
        });
    ...
    onTouchesEnded: function (touch, evt) {
        this.menus.forEach(function (menu) {
            menu.terminadoAlToque(touch, evt);
        });
